...
impl IpPacket {
  ...
  pub fn new(
    ...
    packet.initialize_checksum();

    packet.set_version(4)?;
    let option_len = u8::try_from(option_data.len())?;
    let optional_length_in_words = 1u8 + ((option_len - 1u8) / 4u8);
  ...
  fn initialize_checksum(&mut self) {
    self.set_header_checksum(0u16);
